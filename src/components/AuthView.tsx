  ...
  return (
    <div className="min-h-screen bg-clockit-gradient flex items-center justify-center p-6" id="auth-parent-screen">
      ...
      <div className="w-full max-w-md bg-surface-container-lowest/80 backdrop-blur-xl border border-outline-variant/25 rounded-3xl p-8 shadow-[0_12px_40px_rgba(85,98,77,0.06)] relative z-10 animate-fade-in">
        ...
        {step === 'nickname' ? (
          <form onSubmit={(e) => { e.preventDefault(); if (name.trim()) setStep('auth'); }} className="space-y-6">
            <div className="text-center mb-4">
              <h2 className="text-sm font-headline font-semibold text-primary uppercase tracking-wider">Step 1: Choose Your Nickname</h2>
              <p className="text-xs text-outline mt-1 max-w-[280px] mx-auto leading-relaxed">
                Please enter a nickname first so everything in this quiet space is fully personalized for you.
              </p>
            </div>

            <div className="space-y-1">
              <label className="text-[11px] font-bold text-outline font-label uppercase tracking-widest block px-1">Nickname</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-outline" />
                <input
                  type="text"
                  placeholder="Enter nickname (e.g. Elena, Jack)"
                  value={name}
            ...
            <button
              type="submit"
              disabled={!name.trim()}
              className="w-full bg-primary hover:opacity-95 text-on-primary font-bold py-3.5 px-6 rounded-2xl text-xs font-label uppercase tracking-widest transition-all duration-200 shadow-[0_4px_16px_rgba(85,98,77,0.15)] flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-50 cursor-pointer"
            >
              <span>Continue</span>
              <ArrowRight className="w-4 h-4" />
            </button>

            {/* Quick Demo Autofill option right in step 1 too */}
            <div className="pt-4 border-t border-outline-variant/10 text-center">
              ...
              >
                ...
                <span>Quick demo as Jack</span>
              </button>
            </div>
          </form>
        ) : (
          <div className="animate-fade-in space-y-5">
            {/* Active Nickname Indicator */}
            <div className="bg-primary/5 border border-primary/10 rounded-2xl p-3.5 text-center flex flex-col items-center justify-center">
              <p className="text-xs text-on-surface font-body">
                Step into sanctuary as: <span className="font-extrabold text-primary">{name}</span>
              </p>
              <button
                type="button"
                onClick={() => setStep('nickname')}
                className="text-[10px] uppercase font-mono tracking-widest text-primary hover:underline font-bold mt-1.5"
              >
                Change Nickname
              </button>
            </div>
            ...
            {/* Tab switchers */}
            <div className="flex bg-surface-container-low p-1.5 rounded-2xl mb-4 border border-outline-variant/10">
              <button
                type="button"
                onClick={() => {
                  setIsLogin(true);
            ...
            {errorMsg && (
              <div className="bg-error-container/30 border border-error/20 text-on-error-container p-3.5 rounded-2xl text-xs font-medium text-center mb-4 animate-fade-in flex items-center justify-center gap-2">
